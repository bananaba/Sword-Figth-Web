...
> 마지막 업데이트: 2026-04-29 (Phase 11.6 — 6-도메인 audit pass + 9개 신뢰성 패치 (옵티미스틱 desync, server-clock fallback, useFlash brightness jump, RankedQueue 영속화 + dedupe, unique roomId, matchmake exponential backoff, GPU dispose, HUD aria-live), 45 worker tests)
>
...
### 멀티플레이 / 랭크 (Day 2)
- [x] **Cloudflare RankedQueue Durable Object** (Phase 11a + **11.6**) — `playerId`/rating/name을 받아 ±200 범위 매칭, 매치 성사 시 `ranked-{a}-{b}-{base36(Date.now())}` roomId 발급 (Phase 11.6 unique 접미사). **Phase 11.6**: `state.storage` 영속화 (key `"waiting"`) — DO eviction/Worker 재시작 후에도 큐 보존. 같은 playerId 재폴링 시 dedupe해서 자기 자신 매칭 방지. **잔여**: 30s 대기 시 범위 확장 (P2).
- [x] **Cloudflare DuelRoom Durable Object** (Phase 11a + **11.6**) — WS upgrade, alarm 기반 30Hz tick. `DuelRoomSession`이 hello/ready/guard/attack 처리 + countdown→fighting→roundOver→matchOver 상태머신 + 서버 권위 `resolveAttack`/`applyOutcome` 호출. **Phase 11.6**: roomId timestamp 접미사로 같은 두 플레이어 재매칭 시 별도 DO 인스턴스 보장 (이전 매치 state 잔존 방지). **잔여**: Hibernation API (P2).
...
| **11.6** | **Audit pass + 신뢰성 패치** (Phase 11c 직후, Phase 13 배포 전 안정화): 6개 도메인 병렬 sub-agent 코드 리뷰 (shared/combat, worker DOs, client core, network adapter, FX/visual, docs↔code 정합) + raw code 직접 검증으로 4개 sub-agent 거짓 경보 반박 (resolver block시 `counterUntil = 0` 미설정은 *의도된 동작* — block은 successful landing 아님; `acuteAngleBetween` 영벡터 BLOCK 트리거는 산수 오류 — `π/2 - 0 = 1.57 > 0.52rad` → PIERCE; `initialAiState`의 미전달 tuning은 의도된 디폴트 매개변수; AI `pickThrust` origin은 resolver가 사용하며 `mouseToAttackEvent` 주석이 명시한 의도된 비대칭). **9개 패치 적용**: (a) `useFlash.ts:49` brightness jump — `amount: newPeak` → `Math.max(cur.amount, intensity)` (약한 pulse가 진행 중 envelope 위로 점프 안 함) (b) `useRankedMatch` server-clock fallback — `haveServerClock` 플래그로 첫 state 도착 전 stun/cooldown UI를 0으로 클램프 (이전 `Date.now()` fallback이 server 절대 timestamp와 비교돼 spurious 큰 값) (c) `useRankedMatch` 옵티미스틱 desync — `localPlayerAttackConfirmed` ref. 서버는 모든 outcome(miss/rejected 포함)에 `impact` broadcast하므로 `attackerSide === ourSide` 인 impact가 `impactAt + 220ms` 안에 안 오면 server reject로 간주 → visual 조기 클리어 (d) `RankedQueue` roomId timestamp — `ranked-{a}-{b}-{base36(Date.now())}`로 같은 두 플레이어 재매칭 시 별도 DO 인스턴스 (e) `RankedQueue` 영속화 — `state.storage` lazy load + 변경 시 `put(WAITING_KEY, waiting)`. DO eviction 후에도 큐 보존. 같은 playerId 재폴링 시 dedupe (자기 매칭 방지). 신규 테스트 2개 (영속, dedupe) (f) `pollUntilMatched` exponential backoff — 500ms → 8s cap (2^n). 4xx/abort 즉시 throw, 5xx + 네트워크 글리치 retry (g) `SparkParticles` GPU dispose — `useEffect` cleanup에서 imperative `useMemo` 풀의 geometry/material 회수 (R3F는 JSX 자식만 자동) (h) `DuelDebug.HitboxWire` 동일 패턴 dispose (D키 토글) (i) `DuelHud` sr-only `aria-live="polite"` announcement region — countdown/round-over/match-over에만 자연어 송출, frame 필드는 deps 제외 (fight 도중 spam X). **3개 false-positive 검증으로 패치 보류**: `/result` 인증 — `worker/src/index.ts`에 `/result` 라우트 미노출, DO stub 호출만 가능 → 외부 공격면 없음; room_full 정리 — `socket.close` → `close` listener → `detach`가 정상 작동; TitleScreen focus trap — 풀화면에 모달 패턴 부적절. 누적: 42 → **45 worker tests**, client typecheck/build 모두 통과 |
