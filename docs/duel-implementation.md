...
> 마지막 업데이트: 2026-05-01 잼 마감 당일 (Phase 18 — 잼 D-day 최종 튠/품질 패스: 8방향 슬래시 클립 + 전용 Stun reaction(`anim_hit.fbx` → `Stun.fbx`), Mixamo hips root-motion 25% 댐핑(클립 슬라이드 ↔ 게임 좌표 어긋남 fix), 봇 가드를 8 PI/4 snap → 연속 각도 + critical-damped 스무딩 + 손-tremor wobble + 35% 미러-react로 자연화, 옵저버 perspective에서 상대 가드/blade tip을 X축 미러링(facing flip 회귀 fix), 솔로 매치 BGM이 replay 진입 시 다시 재생되도록 `playBgm("match")` 트리거 명시, 넉백 전 프리셋 0.75× 재튠(BASIC 4/5/6 → 3.0/3.75/4.5 등), `INITIAL_*POS` ±1.0 → ±1.5(시작 거리 3.0 unit), `index.html` title "Chambara Duel — Vibe Jam 2026". Phase 17 Audio 통합(Howler 21 SFX + 4 BGM + ambient + reactive saber hum + Credits 모달)·Phase 16 다중 무기/사이파이 챔버·Phase 13 라이브 배포(Workers `chambara-ranked-worker.200tiger1.workers.dev` + Pages `chambara-duel.pages.dev`) 그대로 유효)
>
...
worker/src/            # Cloudflare Worker + Durable Objects 권위 랭크 서버 (Phase 11a-c/11.6)
  ...
  Fighter.tsx           # xbot/ybot FBX + Mixamo 애니 + 검(grip/tip pose) + drei `<Trail>` + 스턴 별(head bone tracking) + 가드 lean(disc 가변) + 클립 timeScale 압축. **Phase 18**: 슬래시 8방향 클립(`Slash_{0..315}.fbx`) + 시작점 방향 매핑 + 225° 미러 → 315° 합성, `Stun.fbx` 전용 reaction(이전 anim_hit.fbx 폐기), `dampenRootMotionXZ`로 hips position 트랙 X/Z 증분 25% 스케일(in-place 보정 — slash/thrust 클립의 한 발 슬라이드 fix).
  ImpactRings.tsx       # outcome 발화 시 확장 링 풀 (Phase 8) — useImpacts 스토어 구독
  AudioToggle.tsx       # 우상단 mute/unmute 토글 (Phase 17, db003db)
  audio/index.ts        # `initAudio()` user-gesture unlock + preload + 통합 export (Phase 17)
  audio/sfx.ts          # Howler 21개 SFX 사전 로드 + outcome 매핑 + ±5-10% pitch 랜덤 + 변형 random pick (Phase 17)
  audio/bgm.ts          # BGM 4트랙 streaming (`title`/`match`/`victory`/`defeat`) + 600ms crossfade + ducking API (Phase 17). Phase 18 (18bb3fa): 솔로 replay 진입 시 `playBgm("match")` 명시 트리거.
  audio/ambient.ts      # arena ambient 2 loop(chamber + pit_sizzle) + fade (Phase 17)
  audio/saberHum.ts     # Web Audio 합성 reactive hum — 무기/캐릭터별 프로파일 + setActive/setGuard/setStunned/setTimeScale/pulseClash API (Phase 17)
  audio/credits.ts      # 11개 카테고리 출처/저자/라이센스 데이터 (Phase 17 Credits 모달용)
  audio/mute.ts         # mute 상태 + Howler.volume 통합 토글 (Phase 17)
  ...
  ai.ts                 # 봇 의사결정 (가드 각도, 슬라이스/찌르기, smart-slice). **Phase 18 (ddd57e4)**: 가드 각도가 8 PI/4 snap → `targetGuardAngle`/`guardAngle` pair + critical-damped exp decay(`responsiveness=7.5`) + angular speed cap(`5.5 rad/s`) + 두 주파수 손-tremor wobble로 연속 추적, 35% 확률로 상대 가드 라인 미러링(react-feel), `tickAi(dt)` 인자가 global timeScale 반영해 hit-stop·KO slow-mo가 봇 손에도 적용.
...
### 3.6 Phase 18 피드백 (잼 D-day 최종 튠 — 2026-05-01)

| # | 피드백 | 수정 |
|---|---|---|
| 1 | 슬래시가 모두 같은 모션이라 8방향 입력이 시각으로 안 드러남 | `Fighter.tsx`에 8개 새 FBX 클립(`Slash_{0,45,90,135,180,225}.fbx` + 45/225° oneHand·twoHands variant)을 추가. 마우스 드래그 start→end의 *시작점 방향각* 0..360°를 8 슬롯에 nearest-neighbor 매핑. 270°는 225°/315°가 동률이라 안전한 225° 재사용, 315°는 전용 클립 없어서 225° X축 미러링으로 런타임 합성(bone 좌우 swap + position.x / quaternion.y,z 부호 반전). 45°/225°는 무기에 따라 oneHand(rapier) vs twoHands(basic/charge) variant 분기. (e959f05) |
| 2 | 가드 스턴 시 별도 reaction 클립이 필요 | `anim_hit.fbx`(스캐폴드 placeholder) → `Stun.fbx` 전용 클립으로 hit_guard / hit_taken 두 슬롯 통합. (e959f05) |
| 3 | 슬래시/찌르기 클립이 hips root motion으로 한 발 슬라이딩해서 게임 좌표(`s.posX`)와 시각이 어긋남 | `dampenRootMotionXZ(clip, factor=0.25)` 헬퍼 — `mixamorigHips.position` 트랙의 X/Z 변위를 첫 프레임 기준 *증분*에 대해 25%로 곱한 새 AnimationClip 생성. Y(점프) 보존, 첫 프레임 절대 위치 보존(idle ↔ attack 전환 시 점프 방지). slash 0..225° 전 클립 + thrust + 합성 315°에 일괄 적용. (6c01bcc) |
| 4 | 봇이 PI/4 8방향 snap으로 가드를 갈아끼워서 반응이 디지털해 보임 | `ai.tickAi`가 `targetGuardAngle`/`guardAngle` pair로 분리. 매 프레임 critical-damped exp decay(`responsiveness=7.5`, `1 - e^{-k·dt}`) + angular speed cap(`5.5 rad/s`) + 두 주파수 손-tremor wobble(amp 0.07 rad / 1.3 Hz · 2.21 Hz)로 부드럽게 회전. `pickGuardAngle`은 cardinal ±35° spread 연속 샘플(reject sampling: 미세 변경·∼180° flip 60% reject). 35% 확률로 enemy guard 방향 미러링 → react-feel. `tickAi(dt)`가 global timeScale 반영해서 hit-stop·KO slow-mo가 봇 손에도 작동. (ddd57e4) |
| 5 | 옵저버 perspective에서 상대 가드 fan / blade tip이 권위 가드 방향과 어긋남 | `useRankedMatch.ts`에 `mirrorFighterNetStateX` / `mirrorGuardX` / `mirrorVecX` 헬퍼 통합. `flipFacing`(서버가 우리에게 opponent로 표시한 슬롯) 분기에서 posX/velX 외 `guard.{grip,tip}`과 `bladeTip`도 X축 미러링. 이전엔 posX만 미러링되고 가드/blade tip은 raw 값이라 카메라 우측 가드를 좌측에 그렸음. (14bbe39) |
| 6 | 시작 거리(±1.0)가 카메라 앵글에서 너무 가까워 보임 | `INITIAL_PLAYER_POS` -1.0 → -1.5, `INITIAL_OPPONENT_POS` +1.0 → +1.5 (총 2.0 → 3.0 unit). `worker/test/duel-session.test.js`의 spawn 어서션도 hardcode 대신 import한 상수 사용으로 동시 수정 (219dc62, 91a1bd9). |
| 7 | 넉백이 너무 강해 한두 합에 ringout 됨 | 1차 시도(18bb3fa): 0.5×로 절반 — KO가 거의 안 일어남. 2차(007b9d8): `0.75×` 미드포인트로 정착 → BASIC 4/5/6 → 3.0/3.75/4.5, CHARGE 3/7.5/5 → 2.25/5.625/3.75, RAPIER 3/4/9 → 2.25/3.0/6.75. worker test fixture도 갱신. |
| 8 | 솔로 매치를 끝낸 뒤 replay 진입해도 BGM이 안 켜짐 | `useDuelLoop.ts`의 매치 시작 transition에서 `playBgm("match")` 명시 호출 추가. 이전엔 mount 시 1회만 큐잉돼서 같은 컴포넌트 reuse 시 다시 안 트리거. (18bb3fa) |
| 9 | 페이지 title이 잼 출품작임을 알 수 없음 | `client/index.html`의 `<title>`을 "Chambara Duel — Vibe Jam 2026"으로 변경. (60827c8) |
...
`ai.tickAi(now, dt, ...)`는 매 프레임 호출되어:
1. 스턴 중이면 가드 비활성 + 무행동
2. `nextGuardChangeAt` 도달 시 새 **`targetGuardAngle`** 샘플(연속 각도, ±35° spread cardinal jitter, 큰 flip 60% reject) — **Phase 18부터 PI/4 snap 폐기**
3. 매 프레임 `targetGuardAngle`로 critical-damped 스무딩(`responsiveness=7.5`) + angular speed cap(`5.5 rad/s`) + 두 주파수 손-tremor wobble(amp 0.07 rad / 1.3·2.21 Hz) 적용 → 봇 손이 텔레포트 없이 자연 arc로 회전. 35% 확률로 상대 가드 라인 미러링 → react-feel
4. `nextAttackAt` 도달 또는 counter window 활성 시 공격 생성
   - 25% (`thrustChance`): 좌/우에서 가로 찌르기
   - 75%: 슬라이스 — 40% 확률로 player 가드와 평행 각도 선택 (smart-slice = pierce 노림)

생성된 공격은 `commitPending`을 거쳐 동일한 windUp 사이클 적용 → 플레이어가 280ms windUp 동안 보고 가드 조정 가능. `dt`는 global timeScale을 반영해서 hit-stop·KO slow-mo 동안 봇 손이 함께 느려진다 (Phase 18, ddd57e4).

`AiTuning` 객체로 난이도 노브 (현재 단일 프리셋 `DEFAULT_AI`, Phase 18에서 가드 스무딩 4 필드 추가). 랭크 도입 시 시드 RNG로 결정론화 필요.
...
### 넉백 — slice < counter < thrust 위계 (Phase 18 재튠 — Phase 16 값의 0.75×)

| 노브 | BASIC | CHARGE (counter specialist) | RAPIER (thrust specialist) | 변위 (n/FRICTION 5.0) |
|---|---|---|---|---|
| `sliceKnockback` | 3.0 | 2.25 ↓ | 2.25 ↓ | BASIC 0.6 unit |
| `counterKnockback` | 3.75 | **5.625 ↑↑** (counter specialist) | 3.0 ↓ | BASIC 0.75 |
| `thrustKnockback` | 4.5 | 3.75 ↓ | **6.75 ↑↑** (slice arc 밖에서 찌르기) | BASIC 0.9 |

- `attackerFollowFraction` (1.0, 모든 프리셋 공통) — push-along 비율. 1.0 = 풀 추격으로 거리 보존 (Phase 7).

> Phase 16 값(BASIC 4/5/6, CHARGE 3/7.5/5, RAPIER 3/4/9)이 너무 강해 한두 합에 ringout 되는 케이스가 잦았다. Phase 18 18bb3fa에서 `0.5×`로 절반 튠 → 맞으면 KO 안 나는 문제 → 007b9d8에서 `0.75×`(원본의 75%, 즉 0.5×와 1.0× 중간)로 최종 정착. 변위가 BASIC 기준 1.6/2.0/2.8 → 0.6/0.75/0.9 unit. ARENA_RADIUS 4.0에서 mid-arena 한 방 KO는 RAPIER thrust + CHARGE counter만 가능, BASIC은 2-3히트 페이스.
...
- `INITIAL_PLAYER_POS = -1.5`, `INITIAL_OPPONENT_POS = +1.5` — 동일 파일. **Phase 18 (91a1bd9)**: 시작 거리 ±1.0 → ±1.5(총 2.0 → 3.0 unit) — 카메라 앵글이 너무 가까워 보이는 것을 완화. 서버 권위/솔로/시각 모두 같은 상수라 회귀 영향 없음.
...
| **18** | **잼 D-day 최종 튠/품질 패스** (2026-05-01, §3.6 피드백 1-9): (a) **8방향 슬래시 클립** (e959f05) — `Slash_{0,45,90,135,180,225}.fbx` 새 클립 + 45°·225°는 oneHand/twoHands variant(weaponId 분기). 슬래시 시작점 방향각으로 nearest-neighbor 매핑. 270°는 225°/315° 동률이라 225° 재사용, 315°는 225° X축 미러링(bone 좌우 swap + position.x / quaternion.y,z 부호 반전)으로 런타임 합성한 새 AnimationClip. **Stun reaction**: hit_guard/hit_taken 슬롯이 placeholder `anim_hit.fbx` → 전용 `Stun.fbx`로 통일. (b) **hips root motion 댐핑** (6c01bcc) — `dampenRootMotionXZ(clip, 0.25)` 헬퍼 — Mixamo `mixamorigHips.position` 트랙의 X/Z 변위를 첫 프레임 기준 *증분*에 대해서만 25%로 스케일(Y 보존, 첫 프레임 절대 위치 보존). 모든 slash/thrust/합성 315° 클립에 일괄 적용. 이전엔 게임 좌표는 그대로인데 시각상 한 발 앞으로 슬라이딩 → idle ↔ attack 전환 시 jump 발생. (c) **봇 가드 스무딩** (ddd57e4) — `AiState`에 `targetGuardAngle` + `wobblePhase` 필드 추가, `tickAi` signature에 `dt` 인자 추가. 가드 각도가 8 PI/4 snap → critical-damped exp decay(`1 - e^{-7.5·dt}`) + angular speed cap(5.5 rad/s) + 두 주파수 손-tremor wobble(amp 0.07 rad / 1.3 Hz·2.21 Hz). `pickGuardAngle(currentTarget, enemyGuard)`이 cardinal ±35° spread 연속 샘플 + reject sampling(미세 변경·∼180° flip 60% reject) + 35% 확률 enemy guard 미러 react. `Duel.tsx`에서 `dt`는 `useTimeScale.scale * raw_dt`라 hit-stop·KO slow-mo가 봇 손에도 적용. (d) **옵저버 가드/blade tip facing flip 보정** (14bbe39) — `useRankedMatch.ts`에 `mirrorFighterNetStateX` / `mirrorGuardX` / `mirrorVecX` 헬퍼 통합. flipFacing 분기(서버가 우리를 opponent 슬롯에 둔 매치)에서 posX/velX 외 `guard.{grip,tip}`과 `bladeTip`까지 X축 미러링. 이전엔 가드 fan UI가 권위 가드 라인과 좌우 어긋남. `Duel.tsx`도 32라인 추가로 GuardSnapshot에서 `guardDir = rotate90(aim)` 역산 패턴 통일. (e) **spawn 거리 확장** (91a1bd9, 219dc62) — `INITIAL_PLAYER_POS` -1.0 → -1.5, `INITIAL_OPPONENT_POS` +1.0 → +1.5(총 거리 3.0). worker test fixture가 hardcode 대신 import 상수 사용으로 동시 수정. (f) **넉백 0.75× 재튠** (18bb3fa, 007b9d8) — 1차 0.5× 시도 후 KO가 안 일어나서 0.75× 미드포인트로 정착. 모든 프리셋 일괄 곱(BASIC 4/5/6 → 3.0/3.75/4.5, CHARGE 3/7.5/5 → 2.25/5.625/3.75, RAPIER 3/4/9 → 2.25/3.0/6.75). worker test fixture 동시 갱신. (g) **솔로 replay BGM** (18bb3fa) — `useDuelLoop.ts` 매치 시작 transition에 `playBgm("match")` 명시 호출 추가(이전엔 component mount 시 1회만 큐잉). (h) **페이지 title** (60827c8) — `client/index.html` `<title>`을 "Chambara Duel — Vibe Jam 2026"으로. 모든 변경 typecheck pass + worker tests green. |
| **17** | **Phase 9 Audio 통합** (2026-05-01, `claudedocs/research_audio_sfx_bgm_20260430.md`, `docs/jam-polish-plan.md` §Phase 9): (a) 사용자 다운로드 raw 자료(BGM 4개 / ambient 2개 / grunt / round bell / pit_impale + Kenney UI/Sci-fi/Impact 3팩 + Sonniss GameAudio GDC2026 5번들)를 `audio-source/`로 격리(`.gitignore`) — Sonniss 7.5GB가 `client/public/`에 잘못 들어와 있어 빌드 dist가 7GB+로 부풀어오를 위험을 사전 차단. (b) ffmpeg 기반 일괄 정규화: BGM(`bgm_title`/`match`/`victory`/`defeat`)은 `loudnorm I=-16 TP=-1`로 -16 LUFS 통일, `victory`/`defeat`은 7s + 1s fade-out으로 자르기(원본 83-108s가 stinger로는 너무 김). ambient(`chamber` 15s loop, `pit_sizzle` 8s loop)는 fade-in/out 0.4-0.6s + 96kHz/24-bit→44.1kHz/16-bit OGG q4. SFX(grunt/bell/pit_impale)는 peak normalize -1~-3 dB. **`ambient_pit_sizzle`는 mean -36 dB 그대로 보존**(사용자 의도: 배경에 작게 깔리는 톤). 결과 27 OGG 파일 / `client/public/audio/` 4.1MB(원본 240MB+에서 압축). (c) Kenney 픽: UI hover/click/back/match-found/countdown_beep, impact block_a/b/pierce/hit_a/b, sci-fi saber_swing_a/b/c·windup·counter_open, footstep_a/b/c — 18개 추가 OGG 정규화. (d) **`client/src/duel/audio/` 모듈 신설**: `sfx.ts`(Howler 21개 SFX 사전 로드 + outcome 매핑 + ±5-10% pitch 랜덤 + 변형 랜덤 픽), `bgm.ts`(BGM 4트랙 streaming + crossfade 600ms + `playBgm("title"/"match"/"victory"/"defeat")` + ducking API), `ambient.ts`(chamber + pit_sizzle 무한 loop + fade), **`saberHum.ts`** (Web Audio 합성 — 무기별 프로파일(BASIC sine 65Hz / CHARGE sub 32Hz 추가 + LFO 7Hz / RAPIER 90Hz Q 1.8) + 캐릭터별 +5Hz offset(magenta) + sine/saw/triangle 3-osc + biquad lowpass + LFO tremolo. **reactive API**: `setActive`/`setGuard`(filter +400Hz / LFO ×1.5)/`setStunned`(pitch -10% + filter 600Hz)/`setTimeScale`(KO slow-mo 시 LFO 감속)/`pulseClash`(BLOCK 시 1500Hz sweep 0.05s)/`dispose`. Web Audio AudioContext 미준비 시 no-op 폴백.), `credits.ts`(11개 카테고리 출처/저자/라이센스 데이터), `index.ts`(`initAudio()` user-gesture unlock + 사전 preload + 통합 export). (e) **통합 포인트**: `dispatchImpactFx.ts` → `playOutcomeSfx(kind)` 1줄 추가(BLOCK→block_a/b 변형 / HIT→hit_a/b + grunt 70% / PIERCE→pierce + grunt 85% / KO→pit_impale + grunt). `useDuelLoop.ts` countdown phase에 `playCountdownBeepSfx(pitch 0.9→1.0→1.15)` 매 초 + countdown→fighting 전환 시 `playRoundBellSfx()` + matchOver 진입 시 `playBgm(winner==="player" ? "victory" : "defeat")`. `useRankedMatch.ts` "match_over" 메시지 시 `playBgm(victory/defeat)`. `Duel.tsx` GameStage가 player saberHum 마운트 — `weaponId` 변경 시 dispose+재생성, `useFrame` 매 프레임 `s.guard.active`/`s.stunned`/`useTimeScale.scale` 반영. DuelGame/RankedDuelGame mount → `playBgm("match")` + `startArenaAmbient()`, unmount → `playBgm("title")` + `stopArenaAmbient()`. (f) **TitleScreen** mount → `playBgm("title")`(Howler가 첫 user gesture까지 자동 큐잉). character/weapon/Solo/Ranked/Join/Leaderboard 버튼에 `onMouseEnter` ui_hover + onClick ui_click. handleStart 첫 호출 시 `initAudio()`로 Web Audio context resume. 신규 **Credits 모달** 컴포넌트 — title 하단 "Credits" 버튼 → 11개 카테고리 출처(Tallbeard CC0 / Kenney CC0 / Pixabay / Freesound / Sonniss / Web Audio 합성) 리스트 + 외부 링크 + ESC/배경 클릭 닫기. (g) howler@2.2.4 + @types/howler@2.2.12 의존성 추가. audiosprite는 npm 패키지 인식 문제로 스킵하고 개별 Howl 21개로 진행 — HTTP/2 multiplex로 동시 다운 OK, 잼 데모 시연용 충분. (h) typecheck + production build 통과(client dist 146MB, audio 4.1MB만 포함). **미통합 (Phase 17.x 후속 폴리시)**: footstep(캐릭터 이동 거리 누적 트리거 미연결), opponent saberHum(player만 있음 — 두 hum 혼합 시 noisy 우려), `pulseClash` BLOCK outcome 자동 호출(현재 API만 노출), windup SFX(공격 텔레그래프 hook 필요), ranked match-found notification(서버 phase 전환 메시지 추적 필요). |
