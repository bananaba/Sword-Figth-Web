...
| `Arena3D.tsx` | 사이파이 인더스트리얼 듀얼 챔버: 페데스탈 cylinder(`PIT_LEVEL = -3.8`까지 내려가는 3.8m gunmetal 기둥 `#202530`) + 내부 디스크(`#3a4554` 쿨 스틸) + 외곽 림(`#15191f`) + **시안 페리미터 ring(toneMapped:false, intensity 0.55)** + **mid-radius 시안 그리드 ring(intensity 0.32)**. `WALL_INNER_RADIUS`에 gunmetal pit 라이너 + 데크 립을 따라 시안 emissive 스레드. `<Spikes>` + 조명. **분위기**: bg `#04070c`(딥 블루-블랙) + fog 28–60 + ambient `#3e4a5c` 0.55 (쿨 그레이-블루) + key `#dde0e8` 0.95 (moonlight) + cool fill `#5cc7ff` 0.4 (시안 림 라이트, 라이트세이버 + robot 톤 매칭) + **pit 중앙 pointLight `#ff4520` intensity 3.2 distance 14** (빨간 ember 만 universal danger 시그널로 유지). 4-tier 콜로세움 sandstone 톤은 모두 gunmetal로 시프트(`#262d36` → `#2c323b` → `#363d49` → `#404857` → `#1c2128` → `#4d5765`). 18 spectator arch는 **18 NeonPylon으로 교체** — 각 파일런이 gunmetal box(`#2c323b`) + 캡 plate(`#3a414b`) + 수직 시안 emissive 스트립(`#5ed3e6` intensity 1.1) + 상단 비콘 dot로 구성, 멀리서 floodlight ring처럼 보임. **장식 레이어 추가**: (1) 4 카디널 `SentinelSpire` 타워(`WALL_OUTER_RADIUS+2.2` 외곽, base+mid+spire+beacon 4단, 비콘이 시안/마젠타 교차 — 두 플레이어 accent 둘 다 스카이라인에 등장), (2) 9 `DeckHex` 패널(데크 floor 중앙 반경 `(WALL_INNER+WALL_OUTER)/2`, 파일런 사이 위치, gunmetal 헥스로 floor 텍스처 분산), (3) 외벽 horizontal `torusGeometry` 컨듀잇 ring 2개(y=1.05/0.55, 메탈릭 brushed steel — emissive 없음, 인더스트리얼 plumbing 디테일), (4) 6 `FloatingDrone` 부유체(외곽 r=17–18.2, y=4–5.6, 바디 + 시안/마젠타 emissive bead 2개 mesh, fog 띠 안에서 spectator 분위기). **배경 스카이라인** (사용자 피드백 "외벽 위쪽이 빈 검은 하늘이라 허전"): (5) 28 `DistantTower` 도시 실루엣(결정론 mulberry32(0xfaceb00c) 시드, r=26–44, 높이 6–18, 너비 1.6–4.4, 색 3-tone variant `#0a0e15`/`#10151c`/`#161c25`, ~70% 시안/마젠타 수직 emissive 스트립, 약 27% spire cap + 18% antenna whip+beacon으로 탑 변주, fog 28–60 띠 안에서 자연스러운 페이드), (6) 40 `SKY_LIGHTS` 픽셀 비콘(결정론 mulberry32(0xb1ada55), r=22–50, y=3–19, 크기 0.08–0.22 sphere, 시안/마젠타 emissive intensity 1.0–1.8, 멀리 떨어진 factory 창문/floating beacon 분위기). 모든 배경 요소는 정적 — useFrame 비용 0, 외곽 shadow camera 밖이라 그림자 비용도 0. |
| `Spikes.tsx` | 챔바라 가시 함정 — 물 셰이더 폐기 후 대체. 결정론 mulberry32(0xCAFEBA) 시드로 annulus 영역에 area-uniform 분포 480개(밀도 ~1.8 spike/sq unit). drei `<Instances>` 2개 그룹 — 바디(gunmetal industrial pike `#2a3038`, metalness 0.7 / roughness 0.45, CylinderGeometry [0.05, 0.16, 1.30]) + 팁(emissive `#ff3a1a` intensity 1.5, ConeGeometry [0.05, 0.60], `toneMapped: false`로 Bloom 통과 — 빨간 팁은 universal danger 시그널). 각 가시 height/width/yaw/tilt 랜덤 변주. 쿨톤 gunmetal pit floor ringGeometry(`#0d1118`, metalness 0.4)로 베이스 통일. |
